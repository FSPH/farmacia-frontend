...
import { Button, HStack, Input, InputNumber, Pagination, Panel, useMediaQuery } from 'rsuite'
...
export function DepositosCrudPage({
  ...
  const [deleteCandidate, setDeleteCandidate] = useState<DepositoRecord | null>(null)
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async () => {
      alert.success('Deposito salvo', 'Registro atualizado com sucesso.')
      setModalMode(null)
      await queryClient.invalidateQueries({ queryKey: ['depositos-list'] })
    },
    onError: (error: Error) => {
      alert.error('Erro ao salvar Deposito', getErrorMessage(error))
    },
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async () => {
      alert.success('Deposito excluido', 'Registro removido com sucesso.')
      setDeleteCandidate(null)
      await queryClient.invalidateQueries({ queryKey: ['depositos-list'] })
    },
    onError: (error: Error) => {
      alert.error('Erro ao excluir Deposito', getErrorMessage(error))
    },
  })
  ...
  const handleOpenRecordModal = async (mode: 'edit' | 'view', record: DepositoRecord) => {
    ...
    try {
      ...
      alert.error('Erro ao carregar Deposito', getErrorMessage(error, 'Falha ao carregar o deposito.'))
      setModalMode(null)
    } finally {
      if (formRequestIdRef.current === requestId) {
        setIsFormLoading(false)
    ...
    <section className="boname-page depositos-page">
        ...
        footer={
          <>
            <Button
              ...
              onClick={() => {
                if (deleteCandidate) {
                  void deleteMutation.mutateAsync(deleteCandidate.dep_id)
                }
              }}
            >
              Excluir registro
      ...
      >
        <div className="boname-page__confirm-card">
          <p>
            Esta acao remove o cadastro <strong>{deleteCandidate?.dep_descr}</strong> e deve ser usada apenas
          ...
          {deleteCandidate ? (
            <dl className="boname-page__confirm-grid">
              <div>
                <dt>ID</dt>
                <dd>{deleteCandidate.dep_id}</dd>
              </div>
              <div>
                <dt>Status</dt>
                <dd>{deleteCandidate.dep_ativo === 1 ? 'Ativo' : 'Inativo'}</dd>
              </div>
            </dl>
          ) : null}
        </div>
